...
> Objetivo: instalar y validar Gravitee APIM en el entorno base, aplicando el baseline de seguridad y los casos B2B/B2C definidos en `test.md` y `checklist.md`.
...
## 1) Instalación de Gravitee (Helm)

> Nota: usar **Gravitee APIM 4.10.3**. Fijar versión exacta del chart e imágenes en `values.yaml` y registrarlo en evidencias.
...
- Ingress (host `apim-gravitee.local`, TLS)
- Recursos (requests/limits)
- Admin creds desde Vault
- Persistence (si aplica)
  
Ruta sugerida: `APIM/gravitee/values.yaml`
- **IMPORTANTE:** Ajustar recursos de Elasticsearch (componente crítico en RAM).
  - Deshabilitar réplicas (single node).
  - Fijar Heap Size: `ES_JAVA_OPTS="-Xms1g -Xmx1g"`
...
helm upgrade --install gravitee gravitee/apim \
  -n apim-gravitee \
